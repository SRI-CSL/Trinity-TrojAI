...
def pre_process_image(img_path, round=2):
...
def cross_entropy(logits, labels):
    ...
    logits: logits from the model (numpy: Nx1)
...
def compute_metrics(scores, labels):
    # scores is Nx1 (logits of trojan)
    # labels in Nx1
    pred_labels = (scores > 0.5).astype("float")
    acc = np.mean(pred_labels == labels) * 100
    fpr, tpr, thresholds = metrics.roc_curve(labels, scores)
    auc = metrics.auc(fpr, tpr)
    ce = cross_entropy(scores, labels)
    return auc, acc, ce
